...
vino_face_net = cv2.dnn.readNet(vino_face_xml, vino_face_bin)
...
#_________________________________________________________________________________________
    def get_frame(self):
        start = time()
        grab, frame1 = video.read()
        if not grab:
            raise Exception('Image not found!')
        frame = cv2.resize(frame1, (608,608), interpolation = cv2.INTER_AREA)

        vino_face_blob = cv2.dnn.blobFromImage(frame1, size=vino_face_net_size, ddepth=cv2.CV_8U)   
        vino_face_net.setInput(vino_face_blob)

        blob = cv2.dnn.blobFromImage(frame1, size=netsize, ddepth=cv2.CV_8U)
        net.setInput(blob)
        out = net.forward()
